...
void main()
{
   ...
   // err = WD_AI_Trig_Config (master_d, ADTRIGMODE, WD_AI_TRGSRC_ANA, ADTRIGPOL, 0, 0.0, 0, 0, 0, 1);
	...
	//master sets the pxitrigger signal to PXI trigger Bus 1
	//WD_SSI_SourceConn (master_d, SSI_TRIG_SRC1);
	err = WD_Route_Signal (master_d, SSI_TRIG_SRC1, PXI_TRIG_1, 1);
    if (err!=0) {
       printf("WD_Route_Signal error=%d", err);
       exit(1);
    }

	//slave receives the pxitrigger signal through PXI Trigger Bus 1
	err = WD_Route_Signal (slave_d, SSI_TRIG_SRC1, PXI_TRIG_1, 0);
    if (err!=0) {
       printf("WD_Route_Signal error=%d", err);
       exit(1);
    }
...
//master wish to receive the pre-data-ready signal from the slave through bus 0
	 err = WD_Route_Signal (master_d, 0x10, PXI_TRIG_0, 0);
     if (err!=0) {
       printf("WD_Route_Signal error=%d", err);
       exit(1);
     }
//slave output the pre-data-ready signal through bus 0
	 err = WD_Route_Signal (slave_d, 0x10, PXI_TRIG_0, 1);
     if (err!=0) {
       printf("WD_Route_Signal error=%d", err);     
	   exit(1);
     }
	...
	err = WD_AI_Config (slave_d, WD_IntTimeBase, 1, WD_AI_ADCONVSRC_TimePacer, 0, BUFAUTORESET);
    if (err!=0) {
       printf("WD_AI_Config error=%d", err);
       exit(1);
    }
